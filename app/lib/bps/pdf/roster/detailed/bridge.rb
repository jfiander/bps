        ...
        private
          ...
          def bridge_title
            bounding_box([0, 540], width: 325, height: 25) do
              text(
                "Elected Officers for #{Time.zone.today.strftime('%Y')}",
                size: BPS::PDF::Roster::Detailed::SUBTITLE_SIZE, style: :bold, align: :center
              )
            end
          end
          ...
          def bridge_box_email(bridge_office, y_pos)
            bounding_box([175, y_pos], width: 150, height: 40) do
              formatted_text(
                ...
                size: BPS::PDF::Roster::Detailed::SECTION_TITLE_SIZE,
          ...
          def contact_box_committee(name, chair_name, y_pos)
            bounding_box([0, y_pos], width: 175, height: 40) do
              formatted_text(
                ...
                size: BPS::PDF::Roster::Detailed::HEADING_SIZE, align: :left, valign: :center
              )
            end
          end
          ...
          def contact_box_email(email, y_pos)
            bounding_box([175, y_pos], width: 150, height: 40) do
              formatted_text(
                [{ text: email }],
                size: BPS::PDF::Roster::Detailed::HEADING_SIZE, align: :center, valign: :center
              )
            end
          end
        end
