      ...
      class Detailed
        ...
        private
          ...
          def award_block_body(name, recipient, additional)
            h_size = BPS::PDF::Roster::Detailed::HEADING_SIZE
            text "#{name} Award", size: h_size, style: :bold, align: :center
            move_down(10)
            text(
          ...
          def award_recipients(name, recipient, additional)
            ...
            text(
              "This year's #{winner}:",
              size: BPS::PDF::Roster::Detailed::BODY_REG_SIZE, style: :bold, align: :center
            )
          ...
          def current_recipient(name, recipient, additional)
            bounding_box([0, 130], width: 175, height: 140) do
              insert_image(
                "tmp/run/#{name}.png", fit: [180, 140], position: :center, vposition: :center
              )
            end
          ...
          def award_recipient_names(recipient, additional)
            ...
            formatted_text(
              text, size: BPS::PDF::Roster::Detailed::BODY_REG_SIZE, align: :center, valign: :center
            )
          end
