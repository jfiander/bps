...
module RosterPDF::Detailed::Bridge
  def bridge
    formatted_page do
      bridge_title
      bridge_table
  ...
  def bridge_title
    bounding_box([0, 540], width: 325, height: 25) do
      text "Elected Officers for #{Date.today.strftime('%Y')}", size: RosterPDF::Detailed::SUBTITLE_SIZE, style: :bold, align: :center
    end
  end
  ...
  def bridge_box(office, y_pos)
    ...
    bounding_box([0, y_pos], width: 175, height: 40) do
      formatted_text(
        ...
        size: RosterPDF::Detailed::SECTION_TITLE_SIZE, align: :left, valign: :center
      )
    end
    ...
    bounding_box([175, y_pos], width: 150, height: 40) do
      formatted_text(
        ...
        size: RosterPDF::Detailed::SECTION_TITLE_SIZE, align: :center, valign: :center
      )
    end
  end
  ...
  def contact_box(name, committee, email, y_pos)
    ...
    bounding_box([0, y_pos], width: 175, height: 40) do
      formatted_text(
        ...
        size: RosterPDF::Detailed::HEADING_SIZE, align: :left, valign: :center
      )
    end
    bounding_box([175, y_pos], width: 150, height: 40) do
      formatted_text(
        [{ text: email }],
        size: RosterPDF::Detailed::HEADING_SIZE, align: :center, valign: :center
      )
    end
  end
end
