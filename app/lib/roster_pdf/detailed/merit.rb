...
module RosterPDF::Detailed::Merit
  def merit
  ...
  def member_collection(title, collection, description_key, honor: false, mm: false)
    text title, size: RosterPDF::Detailed::SECTION_TITLE_SIZE, style: :bold, align: :center
    move_down(10)
    text config_text[:merit][description_key], size: RosterPDF::Detailed::BODY_REG_SIZE, align: :justify
  ...
  def statement(title, collection, honor)
    ...
    text "We are #{adj} to have #{collection.count} #{title}:", size: RosterPDF::Detailed::BODY_REG_SIZE, align: :justify
    move_down(10)
  end

  def collection_row(left, right)
    draw_text left&.simple_name, size: RosterPDF::Detailed::BODY_REG_SIZE, at: [20, cursor]
    draw_text right&.simple_name, size: RosterPDF::Detailed::BODY_REG_SIZE, at: [195, cursor]
    move_down(10)
  end
