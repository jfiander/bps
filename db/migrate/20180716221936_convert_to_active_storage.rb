...
class ConvertToActiveStorage < ActiveRecord::Migration[5.2]
      ...
      models.each do |model|
        ...
        model.find_each.each do |instance|
          attachments.each do |attachment|
            next unless instance.send(attachment).exists?
  ...
  def checksum(attachment)
    ...
    url = attachment.s3_object.key
    file = bucket.download(url)
    ...
    raise attachment.s3_object.key.inspect
  end

  def bucket
    @bucket ||= BpsS3.new(:files)
  end
end
