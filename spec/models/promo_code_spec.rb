...
RSpec.describe PromoCode, type: :model do
  ...
  describe 'scopes' do
    before do
      @pending = FactoryBot.create(:promo_code, code: 'pending', valid_at: Time.zone.now + 1.hour)
      @current = FactoryBot.create(:promo_code, code: 'current', valid_at: Time.zone.now - 1.hour)
      @expired = FactoryBot.create(:promo_code, code: 'expired', expires_at: Time.zone.now - 1.hour)
    end
