...
RSpec.describe PastCommander, type: :model do
  describe 'validation' do
    context 'with a user' do
      ...
      it 'should be valid without a name' do
        pc = FactoryBot.build(:past_commander, name: nil, user: @user)
        expect(pc.valid?).to be(true)
      end

      it 'should be valid with a name' do
        pc = FactoryBot.build(:past_commander, name: 'John Doe', user: @user)
        expect(pc.valid?).to be(true)
      end
    end
    ...
    context 'without a user' do
      it 'should not be valid without a name' do
        pc = FactoryBot.build(:past_commander, name: nil, user_id: nil)
        expect(pc.valid?).to be(false)
      end

      it 'should be valid with a name' do
        pc = FactoryBot.build(:past_commander, name: 'John Doe', user_id: nil)
        expect(pc.valid?).to be(true)
      end
    end
  end
